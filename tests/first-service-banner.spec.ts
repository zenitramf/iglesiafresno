...
} from "../src/lib/first-service-banner";
import { gotoReady } from "./ready";

const duringServiceDay = new Date("2026-10-04T17:00:00-07:00");
const justBeforeMidnight = new Date("2026-10-04T23:59:59-07:00");
const afterServiceDay = new Date("2026-10-05T00:00:00-07:00");
...
test.describe("first-service banner date gate", () => {
  ...
  test("is hidden once the date exceeds 4 October 2026", () => {
    ...
    expect(
      isFirstServiceBannerVisible(new Date("2026-10-05T12:00:00-07:00"))
    ).toBe(false);
  });
});

test.describe("first-service banner", () => {
  test("shows the first-service notice at the top of the page", async ({
    page,
  }) => {
    ...
    await expect(banner).toContainText(
      "Primer servicio: domingo 4 de octubre a las 5:00 p. m."
    );
    await expect(
      banner.getByRole("link", { name: "Planea tu visita" })
    ).toHaveAttribute("href", FIRST_SERVICE_BANNER.href);
  ...
  test("appears on inner pages while the date gate is open", async ({
    page,
  }) => {
    await page.clock.install({ time: duringServiceDay });
    await page.clock.resume();
  ...
  test("Planea tu visita goes to /visitanos and the mobile menu still opens", async ({
    page,
  }) => {
    await page.clock.install({ time: duringServiceDay });
    await page.clock.resume();
